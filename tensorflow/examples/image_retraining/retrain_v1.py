...
BOTTLENECK_TENSOR_NAME = 'nn1/reshape:0'#'avgpool0/reshape:0'
...
def create_inception_graph():
  ...
  with tf.Session() as sess:
    ...
    with gfile.FastGFile(model_filename, 'rb') as f:
      ...
      '''
      jpeg_data_tensor = tf.placeholder(tf.string, shape=[])
      decoded_jpg = tf.image.decode_jpeg(jpeg_data_tensor, channels=3)
      float_image = tf.cast(decoded_jpg, tf.float32)
      float_image = tf.expand_dims(float_image, 0)
      resized = tf.image.resize_images(float_image, [224, 224])
      print(tf.shape(float_image, name=None))
      #bottleneck_tensor, jpeg_data_tensor, resized_input_tensor = (
      bottleneck_tensor = (
          tf.import_graph_def(
              graph_def, 
              input_map={JPEG_DATA_TENSOR_NAME: resized},
              return_elements=[BOTTLENECK_TENSOR_NAME]))
      bottleneck_tensor = tf.reshape(bottleneck_tensor, [1, 1024], name=None)
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  with tf.name_scope('input'):
    bottleneck_input = tf.placeholder_with_default(
        ...
        name='BottleneckInputPlaceholder') #shape=[None, BOTTLENECK_TENSOR_SIZE]
